...
file_path = "C:\\Users\\Aditya Jindal\\Documents\\researchClementJ\\server\\data\\benzene.txt"
...
# print("m_plus:",m_plus)
# print("m_square_plus:",m_square_plus)
# print("m_minus:",m_minus)
# print("m_mul:",m_mul)


# degree indices operations
def all_operations(m_plus, m_minus, m_mul, m_square_plus):
    ...
    entropy_matrices = {key: entropy_sum(matrix, all_values[key]) for key, matrix in matrices.items()}
...
matrices2_plus={(matrix+str("_plus")):m_plus*value for matrix,value in matrices2.items()}
...
matrices2_mul={(matrix+str("_mul")):m_mul*value for matrix,value in matrices2.items()}
...
def columnf():
    ...
    entropy_matrices2_sum = {key: entropy_sum(matrix, np.sum(matrix)) for key, matrix in matrices2_plus.items()}
    print(entropy_matrices2_sum)
    entropy_matrices2_mul = {key: entropy_sum(matrix, np.sum(matrix)) for key, matrix in matrices2_mul.items()}
